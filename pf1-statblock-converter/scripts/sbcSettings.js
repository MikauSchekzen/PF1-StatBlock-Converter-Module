...
export class sbcSettings {
    ...
    static updateCustomCompendiums () {
    ...
    static updatePrototypeTokenSettings(tokenSetting, attributeKeys = []) {
        ...
        sbcConfig.options.debug && sbcUtils.log("Default Token Settings: " + JSON.stringify(sbcConfig.options.tokenSettings))
...
export const initializeSettings = async function () {
    ...
    sbcSettings.updateCustomCompendiums()
    
    sbcConfig.options.debug && sbcUtils.log("Initialized Settings: " + JSON.stringify(sbcConfig.options))
